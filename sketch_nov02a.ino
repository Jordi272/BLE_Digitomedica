...
//******************
/*int getInt = input[0]*256*256 + input[1]*256 + input[2];
 * //------------
  int getFloat(byte packet[], byte i){
    union u_tag{
      byte bin[4];
      float num;
    }u;
...
void loop() {
  ...
  if (connected) {
    ...
    std::string var = (pRemoteCharacteristic->readValue());
    for( int i=0; i<var.size(); i++){
      Serial.println(var[i]);
      }
     
     
     //char valor = (byte*)valor_sensor;
     Serial.println(valor_sensor,sizeof(valor_sensor));  //************************
  }else if(doScan){
